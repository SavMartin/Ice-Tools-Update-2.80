bl_info = {
	"name": "Ice Tools",
	"author": "Ian Lloyd Dela Cruz, Bookyakuno (2.8Update) fix:Sav Martin",
	"version": (2, 0, 2),
	"blender": (2, 80, 0),
...
def sw_clipping(obj, autoclip, clipcenter):
	if "Mirror" in bpy.data.objects[obj].modifiers:
		pre_cursor_location = bpy.context.scene.cursor.location.copy()
...
class RetopoSupport(bpy.types.Panel):
	...
	def draw(self, context):
		...
		if context.active_object is not None:
			row = layout.row(align=True)
			row.alignment = 'EXPAND'
			row.prop(bpy.context.object,"show_wire",toggle =False)
			row.prop(bpy.context.object,"show_in_front",toggle =False)
			row.prop(bpy.context.space_data.overlay,"show_occlude_wire", toggle =False)
